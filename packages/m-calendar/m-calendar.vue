<template>
  <view :class="`m-calendar ${customClass}`" :style="customStyle">
    <m-action-sheet
      v-model="pickerShow"
      :duration="duration"
      ...
      custom-class="m-calendar__popup"
      @close="close"
      @after-enter="handleOpened"
      @after-leave="handleClosed"
    >
      <view class="m-calendar__header">
        <view v-if="showTypeSwitch" class="m-calendar__tabs">
          <m-tabs ref="calendarTabs" v-model="currentTab" @change="handleTypeChange">
            <m-tab :title="translate('day')" :name="translate('day')" />
            <m-tab :title="translate('week')" :name="translate('week')" />
            <m-tab :title="translate('month')" :name="translate('month')" />
          </m-tabs>
        </view>
        <view v-if="shortcuts.length > 0" class="m-calendar__shortcuts">
          <m-tag
            v-for="(item, index) in shortcuts"
            :key="index"
            :custom-class="`m-calendar__tag ${index === shortcuts.length - 1 ? 'is-last-tag' : ''}`"
      ...
      <view
        v-if="inited"
        :class="`m-calendar__view  ${currentType.indexOf('range') > -1 ? 'is-range' : ''} ${showConfirm ? 'is-show-confirm' : ''}`"
      >
        <view v-if="isRange(currentType)" class="m-calendar__range">
          <view :class="`m-calendar__range-item ${!calendarValue || !isArray(calendarValue) || !calendarValue[0] ? 'is-placeholder' : ''}`">
            {{ rangeLabel[0] }}
          </view>
          <view class="m-calendar__range-separator"></view>
          <view :class="`m-calendar__range-item ${!calendarValue || !isArray(calendarValue) || !calendarValue[1] ? 'is-placeholder' : ''}`">
            {{ rangeLabel[1] }}
          </view>
        </view>
        <m-calendar-view
      ...
      <view v-if="showConfirm" class="m-calendar__confirm">
        <slot name="confirm-left"></slot>
        <view class="m-calendar__confirm-btn-wrapper">
          <m-button size="large" block :disabled="confirmBtnDisabled" @click="handleConfirm">{{ confirmText || translate('confirm') }}</m-button>
        </view>

<template>
  <view v-if="show" :class="`m-notice-bar ${customClass} ${noticeBarClass}`" :style="rootStyle">
    <template v-if="prefix || defaultIcon">
      <m-icon custom-class="m-notice-bar__prefix" :name="prefix || defaultIcon"></m-icon>
    </template>
    <slot v-else name="prefix"></slot>
    <view class="m-notice-bar__wrap">
      <view class="m-notice-bar__content" :style="animation" @transitionend="animationEnd" @click="handleClick">
    ...
    <slot name="suffix">
      <m-icon v-if="closable" custom-class="m-notice-bar__suffix" name="close" @click="handleClose"></m-icon>
    </slot>
  </view>
</template>
...
const $wrap = '.m-notice-bar__wrap'
const $content = '.m-notice-bar__content'
...
const wrapRect = ref<UniApp.NodeInfo | null>(null)
const contentRect = ref<UniApp.NodeInfo | null>(null)
...
const noticeBarClass = computed(() => {
  ...
  type && noticeBarClasses.push(`m-notice-bar--${type}`)

  if (isHorizontal.value) {
    !wrapable && !scrollable && noticeBarClasses.push('m-notice-bar--ellipse')
  } else {
    noticeBarClasses.push('m-notice-bar--ellipse')
  }

  wrapable && !scrollable && noticeBarClasses.push('m-notice-bar--wrap')

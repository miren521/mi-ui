<template>
  <view>
    <m-toast selector="m-month" />
    <view class="m-month">
      <view class="m-month__title" v-if="showTitle">{{ monthTitle(date) }}</view>
      <view class="m-month__days">
        <view
          v-for="(item, index) in days"
          :key="index"
          :class="`m-month__day ${item.disabled ? 'is-disabled' : ''} ${item.isLastRow ? 'is-last-row' : ''} ${
            item.type ? dayTypeClass(item.type) : ''
          } ${item.customClass || ''}`"
          :style="index === 0 ? firstDayStyle : ''"
          @click="handleDateClick(index)"
        >
          <view class="m-month__day-container">
            <view class="m-month__day-top">{{ item.topInfo }}</view>
            <view class="m-month__day-text">
              {{ item.text }}
            </view>
            <view class="m-month__day-bottom">{{ item.bottomInfo }}</view>
          </view>
        </view>
      </view>
    </view>
...
import mToast from '../../m-toast/m-toast.vue'
...
function getFormatterDate(date: number, day: string | number, type?: CalendarDayType) {
  ...
  if (props.formatter) {
    if (isFunction(props.formatter)) {
      ...
      console.error('[m-design] error(m-calendar-view): the formatter prop of m-calendar-view should be a function')
    }
  }
  return dayObj
}

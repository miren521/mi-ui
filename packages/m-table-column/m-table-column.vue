  ...
  <view class="m-table-column">
    <view v-for="config in cellConfigs" :key="config.rowIndex" :class="config.class" :style="config.style" @click="handleRowClick(config.rowIndex)">
      <template v-if="config.visible">
        ...
        <text :class="['m-table__value', { 'is-ellipsis': ellipsis }]" v-else>{{ config.value }}</text>
      </template>
    </view>
  </view>
</template>
...
import { TABLE_KEY } from '../m-table/types'
import type { SpanMethodResult } from '../m-table/types'

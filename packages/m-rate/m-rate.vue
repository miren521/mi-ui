<template>
  <view :class="`m-rate ${disabled ? 'is-disabled' : ''} ${block ? 'is-block' : ''} ${customClass}`" :style="customStyle" @touchmove="onTouchMove">
    <view
    ...
    >
      <m-icon
        :custom-class="`m-rate__item-star ${rate === '100%' ? 'm-rate__item-star--active' : ''}`"
        :name="isActive(rate) ? activeIcon : icon"
        :custom-style="`${rate === '100%' ? iconActiveStyle : iconStyle} ${iconSize}`"
        @click="handleClick(index, false)"
      />
      <view v-if="props.allowHalf" class="m-rate__item-half" @click.stop="handleClick(index, true)">
        <m-icon
          :custom-class="`m-rate__item-star ${rate !== '0' ? 'm-rate__item-star--active' : ''}`"
          :name="isActive(rate) ? activeIcon : icon"
...
function computeRateList() {
  ...
  if (typeof modelValue !== 'number') {
    console.error('[mi ui] error(m-rate): the value of m-rate should be a number')
    return
  }
...
async function onTouchMove(event: TouchEvent) {
  ...
  const rateItems = await getRect('.m-rate__item', true, proxy)

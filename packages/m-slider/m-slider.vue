<template>
  <view :class="rootClass" :style="rootStyle">
    ...
    <view style="flex: 1" :class="rootClass">
      <!-- #endif -->
      <!-- 最小值文本 -->
      <view v-if="showExtremeValue" :class="`m-slider__value m-slider__value--min`">
        {{ min }}
      </view>
      ...
      <view :id="sliderId" :class="barClass" :style="barStyle" @click.stop="onBarClick">
        ...
        <view :id="trackId" class="m-slider__track">
          <template v-if="hasMarks">
            <view v-for="(mark, index) in scaleList" :key="index" :class="scaleItemClass(mark.val)" :style="scaleItemStyle(mark.position)">
              <view
                v-if="mark.label"
                :class="`m-slider__scale-desc ${props.theme === 'capsule' ? 'm-slider__scale-desc--capsule' : 'm-slider__scale-desc--default'}`"
              >
                {{ mark.label }}
              </view>
            </view>
        ...
        <view :class="lineClass" :style="lineStyle">
          <!-- 单滑块 / 双滑块左侧 -->
          <view
            :class="`m-slider__dot ${range ? 'm-slider__dot--left' : ''}`"
            @touchstart.stop="(e: any) => onTouchStart(e, 0)"
            @touchmove.stop.prevent="onTouchMove"
            @touchend.stop="onTouchEnd"
            @touchcancel.stop="onTouchEnd"
          >
            <view v-if="popoverVisible !== 'never'" class="m-slider__dot-popover" :class="{ 'is-visible': showDotPopover(0) }">
              <text class="m-slider__dot-popover-text">{{ dotDisplayValue(0) }}</text>
              <view class="m-slider__dot-popover-arrow"></view>
            </view>
            <view class="m-slider__dot-slider">
              <m-icon name="multiple-horizontal" size="14px" color="#868a9c" />
            </view>
          </view>
          ...
          <view
            v-if="range"
            :class="`m-slider__dot m-slider__dot--right`"
            @touchstart.stop="(e: any) => onTouchStart(e, 1)"
            @touchmove.stop.prevent="onTouchMove"
            @touchend.stop="onTouchEnd"
            @touchcancel.stop="onTouchEnd"
          >
            <view v-if="popoverVisible !== 'never'" class="m-slider__dot-popover" :class="{ 'is-visible': showDotPopover(1) }">
              <text class="m-slider__dot-popover-text">{{ dotDisplayValue(1) }}</text>
              <view class="m-slider__dot-popover-arrow"></view>
            </view>
            <view class="m-slider__dot-slider">
              <m-icon name="multiple-horizontal" size="14px" color="#868a9c" />
      ...
      <!-- 最大值文本 -->
      <view v-if="showExtremeValue" :class="`m-slider__value m-slider__value--max`">
        {{ max }}
      </view>
      <!-- #ifdef MP-DINGTALK -->
    </view>
...
const sliderId = ref<string>(`m-slider-${uuid()}`)
...
const validStep = computed(() => {
  if (props.step <= 0) {
    console.warn('[m ui] warning(m-slider): step must be greater than 0')
    return 1
  }
  return props.step
})
...
const rootClass = computed(() => {
  const classes = ['m-slider']
  classes.push(`m-slider--theme-${props.theme}`)
  if (props.disabled) classes.push('is-disabled')
  if (props.vertical) {
    classes.push('m-slider--vertical')
  } else {
    classes.push('m-slider--horizontal')
  }
  if (hasMarks.value) classes.push('m-slider--marks')
  if (props.showExtremeValue) classes.push('m-slider--with-extreme')
...
const barClass = computed(() => {
  const classes = ['m-slider__bar', `m-slider__bar--${props.theme}`]
  if (props.disabled) classes.push('is-disabled')
  return classes.join(' ')
})
...
const lineClass = computed(() => {
  const classes = ['m-slider__line', `m-slider__line--${props.theme}`]
  if (!props.range) classes.push('m-slider__line--single')
  if (props.disabled) classes.push('is-disabled')
  return classes.join(' ')
})
...
function normalizeRangeValues(value: number[]): [number, number] {
  if (!Array.isArray(value) || value.length < 2) {
    console.warn('[m ui] warning(m-slider): range value should be an array with at least 2 elements')
    return [props.min, props.max]
  }
...
function scaleItemClass(val: number): string {
  const classes = ['m-slider__scale-item', `m-slider__scale-item--${props.theme}`]

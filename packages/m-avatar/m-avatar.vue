<template>
  <view v-if="isShow" :class="rootClass" :style="rootStyle" @click="handleClick">
    <!-- 默认插槽优先 -->
    <slot v-if="hasDefaultSlot"></slot>
    <!-- 图片 -->
    <m-img v-else-if="src" :src="src" :mode="mode" custom-class="m-avatar__img" @error="handleError" @load="handleLoad" />
    <!-- 文本 -->
    <text v-else-if="text" class="m-avatar__text">{{ text }}</text>
    <!-- 图标 -->
    <m-icon v-else-if="icon" :name="icon" custom-class="m-avatar__icon" />
    <!-- 状态徽标 -->
    <view v-if="status || slots['status-icon']" :class="statusClass" :style="statusStyle">
      <slot name="status-icon">
        <view v-if="status" :class="statusIconClass"></view>
      </slot>
    </view>
  </view>
</template>

<script lang="ts">
export default {
  name: 'm-avatar',
  options: {
    addGlobalClass: true,
    // #ifndef MP-TOUTIAO
    virtualHost: true,
    // #endif
    styleIsolation: 'shared'
  }
}
</script>

<script lang="ts" setup>
import { computed, useSlots, type CSSProperties, ref } from 'vue'
import { addUnit, getPropByPath, isDef, isString, objToStyle } from '../common/util'
import { avatarProps, type AvatarShape, type AvatarSize, type AvatarStatus } from './types'
import { useParent } from '../composables/useParent'
import { AVATAR_GROUP_KEY } from '../m-avatar-group/types'

const props = defineProps(avatarProps)

/**
 * Avatar 组件事件定义
 * - error: 图片加载失败时触发
 * - click: 点击头像时触发
 * - load: 图片加载成功时触发
 */
const emit = defineEmits(['error', 'click', 'load'])
const slots = useSlots()

// 父组件上下文：_internal 用于 avatar-group 内部的溢出计数头像，跳过父组件上下文
const { parent: avatarGroup, index } = props._internal ? { parent: ref(null), index: ref(-1) } : useParent(AVATAR_GROUP_KEY)


/**
 * 计算头像是否应该显示
 * 在 avatar-group 中根据 maxCount 判断是否超出数量限制
 */
const isShow = computed(() => {
  if (!avatarGroup.value) {
    return true
  }
  // 在 avatar-group 中，根据 maxCount 判断
  const maxCount = avatarGroup.value.props.maxCount
  if (!isDef(maxCount)) {
    return true
  }
  const count = typeof maxCount === 'number' ? maxCount : parseInt(maxCount, 10)
  // 检查 count 是否为有效数字
  if (isNaN(count) || count <= 0) {
    return true
  }
  return index.value < count
})

/**
 * 获取头像尺寸
 * 优先级: Props > avatarGroup Props > 默认值
 */
const sizeValue = computed(() => {
  return props.size || (getPropByPath(avatarGroup.value, 'props.size') as string | number | AvatarSize) || 'normal'
})

/**
 * 判断是否使用内置尺寸(large/medium/normal/small)
 */
const isBuiltInSize = computed(() => {
  return isString(sizeValue.value) && ['large', 'medium', 'normal', 'small'].includes(sizeValue.value)
})

/**
 * 获取头像形状
 * 优先级: Props > avatarGroup Props > 默认值
 */
const shapeValue = computed(() => {
  return props.shape || (getPropByPath(avatarGroup.value, 'props.shape') as AvatarShape) || 'round'
})

/**
 * 判断是否存在默认插槽
 */
const hasDefaultSlot = computed(() => !!slots.default)

const rootClass = computed(() => {
  const classes = ['m-avatar', props.customClass]
  classes.push(`is-${shapeValue.value}`)

  // 尺寸类仅预设尺寸
  if (isBuiltInSize.value) {
    classes.push(`m-avatar--${sizeValue.value}`)
  }
  
  // 在 avatar-group 中时，添加 item 类
  if (avatarGroup.value) {
    classes.push('m-avatar__group-item')
    if (avatarGroup.value.props.vertical) {
      classes.push('m-avatar__group-item--vertical')
    }
  }
  return classes.join(' ')
})

/**
 * 根节点样式
 */
const rootStyle = computed(() => {
  const style: CSSProperties = {}
  if (isDef(sizeValue.value) && !isBuiltInSize.value) {
    style.width = addUnit(sizeValue.value)
    style.height = addUnit(sizeValue.value)
    style.fontSize = `calc(${addUnit(sizeValue.value)} * 0.45)`
    if (avatarGroup.value) {
      style['--m-avatar-overlap' as any] = `calc(${addUnit(sizeValue.value)} * -0.25)`
    }
  }

  // 处理层叠效果的 z-index
  if (avatarGroup.value) {
    const cascading = avatarGroup.value.props.cascading
    const maxCount = avatarGroup.value.props.maxCount
    let count = avatarGroup.value.children?.length ?? 0
    if (isDef(maxCount)) {
      const parsedCount = typeof maxCount === 'number' ? maxCount : parseInt(maxCount, 10)
      if (!isNaN(parsedCount) && parsedCount > 0) {
        count = parsedCount
      }
    }
    if (cascading === 'left-up') {
      // 左侧在上：第一个头像（最左边）z-index 最大，显示在最上面
      style.zIndex = count - index.value
    } else if (cascading === 'right-up') {
      // 右侧在上：最后一个头像（最右边）z-index 最大，显示在最上面
      style.zIndex = index.value + 1
    }
  }

  if (props.color) {
    style.color = props.color
  }

  if (props.bgColor) {
    style.background = props.bgColor
    // 有背景色但无文字色时，默认白色
    if (!props.color) {
      style.color = '#fff'
    }
  }

  return `${objToStyle(style)} ${props.customStyle}`
})

/**
 * 状态徽标类名
 */
const statusClass = computed(() => {
  return [
    'm-avatar__status',
    `m-avatar__status--${props.statusPosition}`
  ]
})

/**
 * 状态徽标样式
 */
const statusStyle = computed(() => {
  const style: CSSProperties = {}
  // 可以根据需要添加自定义样式
  return objToStyle(style)
})

/**
 * 状态图标类名
 */
const statusIconClass = computed(() => {
  return [
    'm-avatar__status-icon',
    `m-avatar__status-icon--${props.status}`
  ]
})

/**
 * 处理图片加载失败事件
 * @param {Event} event 失败事件对象
 */
const handleError = (event: Event) => {
  emit('error', event)
}

/**
 * 处理图片加载成功事件
 * @param {Event} event 成功事件对象
 */
const handleLoad = (event: Event) => {
  emit('load', event)
}

/**
 * 处理点击事件
 */
const handleClick = () => {
  emit('click')
}
</script>

<style lang="scss">
@use './index.scss';
</style>
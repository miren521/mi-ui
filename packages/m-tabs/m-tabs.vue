...
async function updateLineStyle(animation: boolean = true) {
  ...
  try {
    ...
    console.error('[m-ui] error(m-tabs): update line style failed', error)
  }
}
...
function getActiveIndex(value: number | string) {
  ...
  if (isNumber(value) && value >= children.length) {
    console.error('[m-ui] warning(m-tabs): the type of tabs\' value is Number shouldn\'t be less than its children')
    value = 0
  }
  // 如果是字符串直接匹配，匹配不到用0兜底
  if (isString(value)) {

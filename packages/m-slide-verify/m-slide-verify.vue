<template>
  <view :class="rootClass" :id="slideVerifyId" :style="rootStyle">
    <!-- 背景提示文字 -->
    <view class="m-slide-verify__text">
      <slot name="text">
        <text class="m-slide-verify__text-inner">
          {{ slideVerifyText }}
        </text>
      </slot>
    </view>
    <!-- 滑过区域，包含滑块 -->
    <view class="m-slide-verify__track" :style="trackStyle">
      <view class="m-slide-verify__track-text">
        <slot name="success-text">
          {{ slideVerifySuccessText }}
        </slot>
      </view>
      <!-- 滑块 -->
      <view class="m-slide-verify__button" @touchstart.prevent="onTouchStart" @touchmove.prevent="onTouchMove" @touchend="onTouchEnd">
        <slot v-if="isPass" name="success-icon">
          <m-icon custom-class="m-slide-verify__button-icon m-slide-verify__button-icon--success" :name="successIcon" :size="successIconSize" />
        </slot>

        <slot v-else name="icon">
          <m-icon custom-class="m-slide-verify__button-icon" :name="icon" :size="iconSize" />
        </slot>
      </view>
    </view>
  </view>
...
const slideVerifyId = ref<string>(`m-slide-verify-${uuid()}`)

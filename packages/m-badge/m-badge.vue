<template>
  <view :class="['m-badge', customClass]" :style="customStyle">
    <slot></slot>
    <view
...
/**
  ...
  const content = computed(() => {
    ...
    if (value && max) {
      ...
      if (!Number.isNaN(numValue) && !Number.isNaN(numMax)) {
        return numMax < numValue ? `${numMax}+` : value
      }
    }
    return value
  })

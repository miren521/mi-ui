<template>
  ...
  >
    <view :class="`m-keyboard ${customClass}`" :style="customStyle">
      <view class="m-keyboard__header" v-if="showHeader">
        <slot name="title" v-if="showTitle">
          <text class="m-keyboard__title">{{ title }}</text>
        </slot>
        <view class="m-keyboard__close" hover-class="m-keyboard__close--hover" v-if="showClose" @click="handleClose">
          <text>{{ closeText }}</text>
        </view>
      </view>
      <template v-if="mode !== 'car'">
        <view class="m-keyboard__body">
          <view class="m-keyboard__keys">
            <m-key v-for="key in keys" :key="key.text" :text="key.text" :type="key.type" :wider="key.wider" @press="handlePress"></m-key>
          </view>
          <view class="m-keyboard__sidebar" v-if="mode === 'custom'">
            <m-key v-if="showDeleteKey" large :text="deleteText" type="delete" @press="handlePress"></m-key>
            <m-key large :text="closeText" type="close" :loading="closeButtonLoading" @press="handlePress"></m-key>
          </view>
        </view>
      </template>
      <template v-if="mode === 'car'">
        <view class="m-keyboard__body">
          <view class="m-keyboard__car-keys">
            <m-key v-for="key in keys" :key="key.text" :text="key.text" :type="key.type" :wider="key.wider" is-car @press="handlePress"></m-key>
          </view>
        </view>
      </template>
    </view>
...
import mKey from './key/index.vue'

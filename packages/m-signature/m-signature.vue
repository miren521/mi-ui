<template>
  <view class="m-signature">
    <view class="m-signature__content">
      <!-- #ifdef MP-WEIXIN -->
      <canvas
        class="m-signature__content-canvas"
        :style="canvasStyle"
    ...
    <view class="m-signature__footer">
      <slot
        name="footer"
        :clear="clear"
      ...
      >
        <block v-if="enableHistory">
          <m-button custom-class="m-signature__button" size="small" plain @click="revoke" :disabled="lines.length <= 0">
            {{ revokeText || translate('revokeText') }}
          </m-button>
          <m-button custom-class="m-signature__button" size="small" plain @click="restore" :disabled="redoLines.length <= 0">
            {{ restoreText || translate('restoreText') }}
          </m-button>
        </block>
        <m-button custom-class="m-signature__button" size="small" plain @click="clear">{{ clearText || translate('clearText') }}</m-button>
        <m-button custom-class="m-signature__button" size="small" @click="confirmSignature">
